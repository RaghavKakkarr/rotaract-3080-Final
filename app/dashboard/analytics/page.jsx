...
export default function GodModeAnalytics() {
  ...
  return (
    // 👇 SPACE FIX: pt-16 (mobile) and md:pt-6 (desktop) to reduce top gap
    <main className="min-h-screen bg-neutral-50 dark:bg-[#050505] text-neutral-900 dark:text-white pt-16 md:pt-6 pb-20 px-4 md:px-6 font-sans relative overflow-x-hidden transition-colors duration-300">
      ...
      <div className="max-w-7xl mx-auto space-y-6 md:space-y-10">
        ...
        <div className="relative z-[99]">
          <Link href="/dashboard" className="inline-flex items-center gap-2 text-neutral-500 hover:text-rose-600 transition-colors text-[9px] md:text-[10px] font-black uppercase tracking-widest bg-white dark:bg-white/5 px-3 py-2.5 md:px-4 md:py-3 rounded-xl border border-neutral-200 dark:border-white/10 shadow-sm">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 md:gap-6">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
            <button 
              ...
              className="bg-neutral-900 dark:bg-white text-white dark:text-black px-6 md:px-8 py-3 md:py-4 rounded-2xl md:rounded-3xl flex items-center justify-center gap-3 hover:bg-rose-600 transition-all shadow-xl active:scale-95"
            >
              <FileText size={16} />
              <span className="text-[9px] md:text-[10px] font-black uppercase tracking-widest leading-none">Global Audit</span>
            </button>
            <div className="bg-white dark:bg-white/5 border border-neutral-200 dark:border-white/10 p-3 md:p-6 rounded-2xl md:rounded-3xl backdrop-blur-md flex items-center gap-4">
                <div className="text-left">
                  <p className="text-[8px] font-black text-neutral-500 uppercase tracking-widest mb-1">Mobilization</p>
                ...
                <div className="h-8 w-[1px] bg-neutral-200 dark:bg-white/10" />
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
          {[
            { label: 'Impact', val: districtData.stats.totalImpact, ic: Users, color: 'text-green-600' },
            { label: 'Hours', val: districtData.stats.hours, ic: Clock, color: 'text-rose-600' },
            { label: 'Efficiency', val: districtData.stats.efficiency, ic: Target, color: 'text-blue-600' },
            { label: 'Density', val: districtData.stats.projectDensity, ic: PieChart, color: 'text-orange-600' },
          ].map((s, i) => (
            <div key={i} className="bg-white dark:bg-white/5 border border-neutral-200 dark:border-white/10 p-4 md:p-8 rounded-[1.5rem] md:rounded-[3rem] shadow-sm group">
               <s.ic className={`${s.color} mb-2 md:mb-3 group-hover:scale-110 transition-transform`} size={20} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
          <div className="bg-white dark:bg-white/[0.02] border border-neutral-200 dark:border-white/10 p-6 md:p-10 rounded-[2rem] md:rounded-[3.5rem] shadow-xl">
            <h2 className="text-lg md:text-xl font-black flex items-center gap-3 text-rose-600 uppercase italic tracking-tighter mb-6 md:mb-8"><Map size={20}/> Zone Performance</h2>
            <div className="space-y-4 md:space-y-6">
               {Object.entries(zoneStats).map(([zone, data]) => (
                 <div key={zone} className="space-y-1.5 md:space-y-2">
                    ...
                    <div className="w-full h-1.5 md:h-2 bg-neutral-200 dark:bg-white/5 rounded-full overflow-hidden">
          ...
          <div className="lg:col-span-2 bg-white dark:bg-white/[0.02] border border-neutral-200 dark:border-white/10 p-6 md:p-10 rounded-[2rem] md:rounded-[3.5rem] shadow-xl">
            <h2 className="text-lg md:text-xl font-black flex items-center gap-3 text-amber-600 uppercase italic tracking-tighter mb-8 md:mb-10"><Trophy size={20}/> Leadership Matrix</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
               {topClubs.map((club, i) => (
                 <div key={i} className="bg-neutral-50 dark:bg-black/40 border border-neutral-200 dark:border-white/5 p-4 md:p-6 rounded-[1.5rem] md:rounded-[2rem] group transition-all">
                    <div className="flex justify-between items-start mb-2 md:mb-4">
                      <span className="text-[8px] font-black text-neutral-400 uppercase">Rank #0{i+1}</span>
                      <div className="flex gap-1 text-amber-500">{[...Array(Math.max(1, 5-i))].map((_, s) => <Star key={s} size={8} fill="currentColor" />)}</div>
                    </div>
                    ...
                    <div className="flex justify-between border-t border-neutral-200 dark:border-white/5 pt-3 md:pt-4">
        ...
        {/* DRR RADAR */}
        <div className="bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-500/20 p-6 md:p-10 rounded-[2rem] md:rounded-[3.5rem] shadow-xl relative overflow-hidden">
           <h2 className="text-xl md:text-2xl font-black flex items-center gap-3 text-red-600 dark:text-red-500 uppercase italic tracking-tighter mb-8 relative z-10"><Fingerprint size={24}/> DRR Radar</h2>
           <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6 relative z-10">
              {riskClubs.map((club, i) => (
                <div key={i} className="bg-white dark:bg-black/60 border border-red-200 dark:border-red-500/10 p-4 md:p-5 rounded-[1.5rem] md:rounded-[2.5rem] flex flex-col justify-between shadow-sm">
                   <div>
                     ...
                     <p className="text-[8px] text-red-600 font-bold uppercase tracking-widest">Zone {club.zone} | Dormant</p>
                   </div>
                   <button className="mt-4 md:mt-6 py-2 md:py-2.5 bg-red-600 text-white rounded-xl text-[8px] font-black uppercase hover:bg-red-700 transition-colors">Request Audit</button>
                </div>
              ))}
           </div>
        </div>
        ...
        {/* BLOOD MATRIX */}
        <div className="bg-white dark:bg-white/[0.02] border border-neutral-200 dark:border-white/10 p-6 md:p-10 rounded-[2rem] md:rounded-[3.5rem] shadow-xl">
           <h2 className="text-lg md:text-xl font-black flex items-center gap-3 text-rose-600 uppercase italic tracking-tighter mb-6 md:mb-8"><Droplets size={20}/> Blood Support Matrix</h2>
           <div className="grid grid-cols-4 md:grid-cols-4 lg:grid-cols-8 gap-3 md:gap-4">
              {['A+', 'A-', 'B+', 'B-', 'O+', 'O-', 'AB+', 'AB-'].map(type => {
                ...
                return (
                  <div key={type} className="bg-neutral-50 dark:bg-black/40 border border-neutral-200 dark:border-white/5 p-3 md:p-6 rounded-2xl md:rounded-3xl text-center group transition-all">
                    <p className="text-lg md:text-2xl font-black text-rose-600 group-hover:scale-110 transition-transform">{type}</p>
                    <p className="text-[8px] text-neutral-500 font-black uppercase tracking-tighter mt-1">{count} U</p>
                  </div>
                );
              })}
           </div>
