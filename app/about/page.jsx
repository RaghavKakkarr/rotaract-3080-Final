...
export default function AboutPage() {
  ...
  return (
    // 👇 FIX: pt-28 for mobile to ensure content clears the fixed Navbar completely
    <main className="min-h-screen bg-neutral-50 dark:bg-neutral-950 text-neutral-900 dark:text-white pt-28 md:pt-32 pb-20 px-4 md:px-6 font-sans transition-colors duration-300 overflow-x-hidden">
      <div className="max-w-6xl mx-auto">
        ...
        {/* THE ROTARY LEGACY - Responsive Grid */}
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-10 md:gap-16 mb-16 md:mb-32 items-center bg-white dark:bg-white/[0.02] border border-neutral-200 dark:border-white/10 p-6 md:p-12 rounded-[2rem] md:rounded-[4rem] shadow-sm">
          <div className="space-y-6">
            ...
            <div className="space-y-4 text-neutral-600 dark:text-neutral-400 leading-relaxed font-medium italic text-sm md:text-base">
              ...
              <p>
                Unki journey 1980s se shuru hui aur aaj wo global Rotary movement ke sabse bade faces mein se ek hain. RID 3080 ki sacchi pehchan Raja Saboo sir hi hain.
              </p>
            </div>
          </div>
          
          <div className="aspect-square bg-neutral-100 dark:bg-neutral-900 rounded-[1.5rem] md:rounded-[3rem] relative overflow-hidden group border border-neutral-200 dark:border-white/10">
            <img 
              src="/rk-saboo.jpg" 
              alt="PRIP Rajendra K. Saboo" 
              className="w-full h-full object-cover grayscale group-hover:grayscale-0 group-hover:scale-105 transition-all duration-1000"
            />
            <div className="absolute bottom-4 left-4 right-4 md:bottom-8 md:left-8 md:right-8 p-4 md:p-6 bg-white/90 dark:bg-black/75 backdrop-blur-md rounded-2xl border border-neutral-200 dark:border-white/10">
        ...
        <section className="mb-16 md:mb-32">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            {[
              ...
              <div key={i} className="bg-white dark:bg-white/5 border border-neutral-200 dark:border-white/10 p-8 rounded-[1.5rem] md:rounded-[2.5rem] hover:border-rose-500 transition-all group">
                <p.ic className="text-rose-600 mb-6 group-hover:scale-110 transition-transform" size={32} />
                <h3 className="text-lg md:text-xl font-black uppercase italic mb-3 group-hover:text-rose-600 transition-colors">{p.t}</h3>
                <p className="text-xs md:text-sm text-neutral-500 dark:text-neutral-400 font-medium leading-relaxed italic">{p.d}</p>
              </div>
            ))}
          </div>
        </section>
              ...
              <div className="grid grid-cols-2 gap-3 md:gap-4">
                {regions.map((r, i) => (
                  <div key={i} className="flex items-center gap-2 md:gap-3 group">
                    <div className="w-2 h-2 bg-rose-500 rounded-full group-hover:scale-150 transition-all shrink-0"></div>
                    <span className="text-[10px] md:text-xs font-black uppercase tracking-widest text-neutral-500 dark:text-neutral-300">{r}</span>
                  </div>
                ))}
              </div>
            </div>
            
            {/* DYNAMIC BOX: Light background in light mode, Dark in dark mode */}
        ...
        {/* 🏺 THE JOURNEY - Responsive Timeline */}
        <section className="bg-white dark:bg-white/[0.03] border border-neutral-200 dark:border-white/10 p-6 md:p-20 rounded-[2rem] md:rounded-[4.5rem]">
          ...
          <div className="max-w-4xl mx-auto space-y-10 md:space-y-16">
            {[
              ...
              <div key={i} className="flex flex-col md:flex-row gap-4 md:gap-16 border-l-2 border-neutral-200 dark:border-white/10 pl-6 md:pl-10 relative group hover:border-rose-500 transition-colors">
                <div className="absolute -left-[9px] top-0 w-4 h-4 bg-rose-500 rounded-full shadow-[0_0_15px_rgba(244,63,94,0.3)]"></div>
                ...
                <div className="space-y-2 pb-2">
                  <h4 className="text-lg md:text-2xl font-black uppercase italic tracking-tight text-neutral-900 dark:text-white leading-none group-hover:text-rose-600 transition-colors">{step.title}</h4>
                  <p className="text-sm md:text-base text-neutral-500 dark:text-neutral-400 font-medium leading-relaxed italic">{step.desc}</p>
                </div>
              </div>
            ))}
          </div>
